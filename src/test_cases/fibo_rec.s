.data
	 ...
	 Main: .word 13, 12, IO, type_Main, Object.abort, Object.type_name, Object.copy, IO.out_string, IO.out_int, IO.in_string, IO.in_int, Main.fibo, Main.main
.text
	 ...
	 sw $t2 0($sp)
	 lw $t0, 0($sp) 
	 bne $t0, $zero, Main.fibo.if2
	 lw $t0, 4($fp)
	 li $t1, 1
	 seq $t2, $t0, $t1
	 sw $t2 8($sp)
	 lw $t0, 8($sp) 
	 bne $t0, $zero, Main.fibo.if6
	 lw $t0, 4($fp)
	 li $t1, 1
	 sub $t2, $t0, $t1
	 sw $t2 20($sp)
